...
exports.getOrder = async (req, res) => {
    try {
        const shops = await Shop.find({owner: req.user._id}).select('_id');
        const shopIds = shops.map(s => s._id);
        const order = await Order.findOne({_id: req.params.id, shop: {$in: shopIds}})
            .populate({path: 'user', select: 'firstName lastName fullName email phone address'})
            .populate({path: 'shop'})
...
exports.updateOrderStatus = async (req, res) => {
    try {
        ...
        const allowedStatuses = ['delivering', 'completed', 'cancelled'];
        if (!allowedStatuses.includes(status))
            return res.status(400).json({message: 'Invalid status. Allowed: delivering, completed, cancelled'});
        const shops = await Shop.find({owner: req.user._id}).select('_id');
        const shopIds = shops.map(s => s._id);
        const order = await Order.findOne({_id: req.params.id, shop: {$in: shopIds}});
        if (!order)
...
exports.updateOrderItemStatus = async (req, res) => {
    try {
        ...
        item.status = status;
        await order.save();
        res.status(200).json({message: `Order item status updated to ${status}`, data: order});
    } catch (e) {
        res.status(500).json({message: e.message});
    }
}
